...
const (
	...
	tcSocketBypassCapacity = 32768
)

const TCRoutingMark uint32 = 1 << 29
...
func PrepareTC(config TCConfig) (*TCBackend, error) {
	...
	_ = raiseMemlockLimit()
	maps, err := loadObjectMaps(loadTC, map[string]mapSpecOverride{
		...
		"tc_socket_bypass":       {name: "sb_tc_socket", mapType: CiliumEBPF.LRUHash, maxEntries: tcSocketBypassCapacity},
	...
	programs, err := loadObjectPrograms(loadTC, maps, []programSelection{
		{section: "classifier/local_egress_ethernet", name: "sb_tc_local_l2"},
		{section: "classifier/local_egress_raw_ip", name: "sb_tc_local_l3"},
		{section: "classifier/shared_ingress_ethernet", name: "sb_tc_share_l2"},
		{section: "classifier/shared_ingress_raw_ip", name: "sb_tc_share_l3"},
		{section: "classifier/delivery_ingress", name: "sb_tc_deliver"},
	})
	if err != nil {
		_ = closeMaps(maps)
		return nil, err
	...
	backend := &TCBackend{
		...
		socketBypassMapFD: maps["tc_socket_bypass"].FD(),
	}
...
func (b *TCBackend) updateControlLocked() error {
	...
	if err := updateMap(b.controlMapFD, unsafe.Pointer(&key), unsafe.Pointer(&b.control)); err != nil {
		return E.Cause(err, "update TC eBPF control")
	}
	return nil
}
...
func (b *TCBackend) RegisterProtectedSocket(cookie uint64) error {
	if cookie == 0 {
		return E.New("invalid TC eBPF socket cookie")
	}
	value := uint8(1)
	b.access.RLock()
	defer b.access.RUnlock()
	...
	if err := updateMap(b.socketBypassMapFD, unsafe.Pointer(&cookie), unsafe.Pointer(&value)); err != nil {
		return E.Cause(err, "register TC eBPF protected socket")
	}
	return nil
}

func (b *TCBackend) LookupAssignment(protocol uint8, source, destination netip.AddrPort, remove bool) (TCAssignment, error) {
	key, err := makeTCAssignKey(protocol, source, destination)
	if err != nil {
...
func makeTCAssignKey(protocol uint8, source, destination netip.AddrPort) (tcAssignKey, error) {
	var key tcAssignKey
