...
MAP(tc_socket_bypass, __u64, __u8, BPF_MAP_TYPE_LRU_HASH, 32768U);
...
INLINE bool socket_bypassed(__u64 cookie) {
    return map_lookup(&tc_socket_bypass, &cookie) != 0;
}
...
NOINLINE int assign_socket(struct __sk_buff *skb, const struct sb_tc_control *control,
    ...
    if (map_update(&tc_assignment, key, &value, BPF_ANY) != 0) {
        sk_release(socket);
        return TC_ACT_SHOT;
    }
...
INLINE int local_egress(struct __sk_buff *skb, bool ethernet) {
    ...
    if (cookie == 0U || socket_bypassed(cookie)) return TC_ACT_UNSPEC;
...
SEC("classifier/local_egress_ethernet")
int singbox_tc_local_egress_ethernet(struct __sk_buff *skb) {
    return local_egress(skb, true);
}

SEC("classifier/local_egress_raw_ip")
int singbox_tc_local_egress_raw_ip(struct __sk_buff *skb) {
    return local_egress(skb, false);
}
