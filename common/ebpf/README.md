...
Forwarded packets are excluded through `ingress_ifindex`; sing-box sockets are
excluded through cookies registered by the process-wide socket control. Selected
packets are addressed to the delivery peer, cross the veth, and are assigned at
...
interface, shared source MAC, and packet path for userspace recovery.
...
UDP replies use transparent sockets bound to the original response source. Each
socket belongs to its userspace UDP session and closes with that session.
...
bit 29 in the packet mark while preserving all other mark bits. Userspace
...
Policy-routing setup holds a process-external lock and rejects unrelated routes
or rules that already reference the dedicated table. Stale matching state from
...
Local egress additionally requires a socket cookie and bypasses protected
cookies. Shared source CIDR and MAC include/exclude policies are evaluated only
on the shared path.
...
| sockets and assignments | `SOCKMAP`, `LRU_HASH` | TCP listeners, protected cookies, and original-flow metadata. |
...
Box construction installs the socket-control wrapper only when local interception
is configured. The protector queues cookies created before the BPF maps are
available and flushes them after backend preparation. Shared-only and unused
builds do not wrap sing-box sockets.

Startup creates listeners, loads maps and programs, registers protected sockets
and TCP listeners, prepares policy routing, creates the delivery link when local
...
the default interface disappears, only local interception is paused.

Shutdown stops network and rule-set callbacks, disables interception, closes
listeners and UDP sessions, unregisters socket protection, detaches filters,
