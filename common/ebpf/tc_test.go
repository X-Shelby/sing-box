...
func TestTCABI(t *testing.T) {
	...
	if size := unsafe.Sizeof(tcAssignKey{}); size != 40 {
		t.Fatalf("unexpected TC assignment key size: %d", size)
	}
	if size := unsafe.Sizeof(TCAssignment{}); size != 12 {
...
func TestMakeTCAssignKey(t *testing.T) {
	for _, test := range []struct {
		...
		key, err := makeTCAssignKey(ProtocolTCP, source, destination)
		if err != nil {
			t.Fatal(err)
		}
		if key.Family != test.family || key.Protocol != ProtocolTCP ||
			key.SourcePort != source.Port() || key.DestinationPort != destination.Port() {
