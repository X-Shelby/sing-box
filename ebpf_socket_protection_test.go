...
func TestPrepareEBPFSocketProtection(t *testing.T) {
	for _, testCase := range []struct {
		...
		{"multiple shared", []option.Inbound{ebpfInbound("shared"), ebpfInbound("shared")}, false},
		{"shared and local", []option.Inbound{ebpfInbound("shared"), ebpfInbound("local")}, true},
	} {
...
func ebpfInbound(mode string) option.Inbound {
	return option.Inbound{
		Type:    C.TypeEBPF,
		Options: &option.EBPFInboundOptions{Mode: mode},
	}
}
