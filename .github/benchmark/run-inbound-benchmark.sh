...
start_sing_box() {
  ...
  case "$variant" in
    ebpf-local|ebpf-profile-local|ebpf-local-validation)
  ...
  ip netns exec "$namespace" "$sing_box" run -c "$config" > "$log" 2>&1 &
  sing_box_pid=$!
  for _ in $(seq 1 30); do
...
reset_router_rules() {
  ...
  ip netns exec "$app_namespace" "$firewall" -t mangle -F OUTPUT
...
set_local_validation_rules() {
  ...
  local network
  for network in tcp udp; do
    ip netns exec "$app_namespace" "$firewall" -t mangle "$rule_action" OUTPUT \
      -m owner --uid-owner 65534 -d "$server_address" \
      -p "$network" --dport "$server_port" \
      -j DSCP --set-dscp "$local_validation_dscp"
    ip netns exec "$router_namespace" "$firewall" -t filter "$rule_action" FORWARD \
      -d "$server_address" -p "$network" --dport "$server_port" \
      -m dscp --dscp "$local_validation_dscp" -j REJECT
  done
}

expect_local_direct_blocked() {
  local scenario=$1
  local report="$output/validation/ebpf-local-direct-$scenario.json"
  if run_benchmark_client ebpf-local-validation "$scenario" 250ms 0s "$report"; then
    echo "ebpf-local direct $scenario traffic escaped the validation blocker" >&2
    return 1
  fi
  jq -e --arg scenario "$scenario" '
    .results | length == 1 and
    .[0].scenario == $scenario and
    (.[0].errors > 0 or .[0].rate == 0)
  ' "$report" >/dev/null
}
...
run_local_leak_check() {
  local variant=ebpf-local-validation
  local repetition=validation
  local result=0
  server_port=20991
  reset_router_rules
  start_server "$variant" "$repetition" || result=$?
  if [[ $result -eq 0 ]]; then
    set_local_validation_rules add || result=$?
  fi
  if [[ $result -eq 0 ]]; then
    expect_local_direct_blocked tcp-short || result=$?
  fi
  if [[ $result -eq 0 ]]; then
    expect_local_direct_blocked udp-pps || result=$?
  fi
  if [[ $result -eq 0 ]]; then
    start_sing_box "$variant" "$repetition" || result=$?
  fi
  if [[ $result -eq 0 ]]; then
    run_benchmark_client "$variant" tcp-short,udp-pps,udp-unconnected-pps,udp-churn \
      1s 0s "$output/validation/ebpf-local-interception.json" || result=$?
  fi
  stop_sing_box
  stop_server
  set_local_validation_rules delete 2>/dev/null || true
  reset_router_rules
  if [[ $result -ne 0 ]]; then
    printf '%s\t%s\t%s\n' "ebpf-local-leak-check" "$repetition" "$result" \
      >> "$output/failures.tsv"
    failed=1
  fi
}
...
run_ebpf_udp_profile() {
  ...
  reset_router_rules
  if [[ $result -eq 0 ]]; then
    go tool pprof -top -nodecount=50 "$sing_box" \
    ...
    go tool pprof -top -nodecount=50 -sample_index=alloc_space \
      ...
      "$sing_box" "$profile_dir/udp-connected-allocs-after.pprof" \
      > "$profile_dir/udp-connected-allocs.txt"
    go tool pprof -top -nodecount=50 -sample_index=inuse_space \
      "$sing_box" "$profile_dir/udp-connected-heap-after.pprof" \
...
if [[ -n ${seen_variants[ebpf-local]:-} ]]; then
  echo "validating ebpf-local interception against direct leakage" >&2
  run_local_leak_check
fi
