...
func (i *Inbound) newTCConnection(
	...
	assignment, err := backend.LookupAssignment(commonEBPF.ProtocolTCP, source, destination, true)
	if err != nil {
...
func (i *Inbound) newTCPacket(
	...
	_, destination, err := packetDestinationsFromOOB(oob)
	if err != nil {
	...
	assignment, err := backend.LookupAssignment(commonEBPF.ProtocolUDP, client, destination, false)
	if err != nil {
	...
	i.udpClientTable.setDirectBinding(client, destination, sourceMAC)
...
func (w *tcPacketWriter) WritePacket(buffer *buf.Buffer, destination M.Socksaddr) error {
	...
	socket, err := w.clientState.replySocket(destinationAddress, w.inbound.newTCUDPReplySocket)
	if err != nil {
		return err
	}
...
func (i *Inbound) newTCUDPReplySocket(source netip.AddrPort) (*net.UDPConn, error) {
	...
	listenConfig := net.ListenConfig{Control: control.Append(adapter.EBPFSocketProtectionControl(i.ctx), func(_ string, _ string, rawConn syscall.RawConn) error {
